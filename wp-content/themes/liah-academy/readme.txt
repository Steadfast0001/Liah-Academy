...
Version: 1.0.0
Description: Custom high-performance premium WordPress theme for Liah Academy, Buea. Supports PHP & PostgreSQL (PG4WP compatible) databases.
...
3. Database Setup (PostgreSQL):
   - This theme is fully compatible with standard MySQL and PostgreSQL.
   - If using PostgreSQL, ensure you have set up a PostgreSQL bridge for WordPress, such as:
     - PG4WP (PostgreSQL for WordPress)
     - Or a custom DB driver in `wp-content/db.php`.
   - On theme activation, the theme will automatically execute standard SQL statements to create the custom student applications table (`wp_liah_applications` or prefix equivalent) and seed initial courses and services.
...
* Unified Sign-Up / Registration: Multi-step registration form that writes to the PostgreSQL table and uploads applicant documentation. Creates a custom user session for status checkins.
